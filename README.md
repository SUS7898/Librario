...
- 📚 **라이브러리 다중 경로 · 라이브러리별 예약** — 한 라이브러리에 여러 폴더를 넣고, 라이브러리마다 스캔 주기를 따로 정할 수 있습니다.
- 🧾 **스캔 대기열** — 스캔 중에 다른 라이브러리를 눌러도 거부되지 않고 대기열에 들어갑니다. 순서 변경·개별 취소·전체 비우기 지원.
- 🔤 **제목 출처 선택** — EPUB·ComicInfo 내부 제목이 권마다 다르게 적혀 `제목 1 / 제목. 2` 처럼 섞이고 순서가 뒤죽박죽이면 **파일명 우선**으로 바꾸세요. (관리 → 예약·규칙 → 제목 출처)
- ♻️ **재적용** — 태그 규칙·제목 설정을 바꾼 뒤 **파일을 다시 읽지 않고** 기존 책에 즉시 반영합니다(라이브러리 목록의 '재적용' 버튼). 전체 재스캔보다 훨씬 빠릅니다.
- 🚫 **태그 제외 폴더** — `보관소` 처럼 분류용 상위 폴더가 전부 태그가 되는 문제를 막습니다.
- 🖱 **라이브러리 드래그 정렬** — 끌어서 순서를 바꿉니다(화면이 다시 그려지지 않음).
- 🏷 **태그 다중 선택 · 형식 필터** — 태그를 여러 개 골라 모두 만족하는 것만 보고, 태그 검색·초기화·형식(만화/PDF/EPUB/텍스트) 필터를 함께 쓸 수 있습니다.
- ↩️ **목록 위치 유지** — 책을 보고 뒤로 나와도 보던 위치로 돌아옵니다.
- 📜 **웹툰 이어보기** — 가운데를 눌렀을 때만 상·하단 바가 나오고, 스크롤이 끝에 닿으면 다음 화가 이어붙어 계속 내려갑니다.

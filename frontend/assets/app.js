...
function activeTab(){
  ...
  if(hp.startsWith('home')) return 'home';
...
async function route(){
  ...
  const hp = location.hash.slice(2);
  clear(view).append(h('div',{class:'spinner'}));
  window.scrollTo(0,0);
  try{
...
function hrow(title, items, node){
  if(!items || !items.length) return null;
  return h('div',{class:'row'},
    h('div',{class:'row-head'}, h('h2',{},title)),
...
async function viewHome(view){
  ...
  const rows = [
    hrow('이어보기', data.continue_reading, bookCard),
    hrow('최근 추가된 책', data.recently_added_books, bookCard),
    hrow('최근 업데이트된 시리즈', data.recently_updated_series, seriesCard),
    hrow('최근 업데이트된 책', data.recently_updated_books, bookCard),
  ].filter(Boolean);
  clear(view);
  if(!rows.length){
...
async function viewSearch(view){
  ...
  const input = h('input',{class:'input', placeholder:'제목, 작가, 태그로 검색', value:q0});
  ...
  let mode='series';
  const results = h('div',{});
  view.append(h('div',{class:'filterbar'}, h('div',{style:{flex:'1',minWidth:'200px'}}, input), modeSeg), results);
  let timer=null;
  ...
  async function go(){
    ...
    if(!q){ clear(results).append(h('div',{class:'center-pad'},'검색어를 입력하세요.')); return; }
    clear(results).append(h('div',{class:'spinner'}));
    const ep = mode==='series'?'/api/series?':'/api/books?';
    const data = await api(ep+'search='+encodeURIComponent(q)+'&size=100');
    clear(results);
    if(!data.items.length){ results.append(h('div',{class:'center-pad'},`'${q}' 검색 결과가 없습니다.`)); return; }
    const grid = h('div',{class:'grid'});
    data.items.forEach(it=>grid.append(mode==='series'?seriesCard(it):bookCard(it)));
    results.append(grid);
  }
  if(q0) go(); else clear(results).append(h('div',{class:'center-pad'},'검색어를 입력하세요.'));
}
...
async function viewSettings(view){
  ...
  view.append(
    ...
    card('만화 뷰어', h('div',{},
      ...
      settingRow('기본 맞춤', segPick(['width','height'],['너비 맞춤','높이 맞춤'],prefs.comicFit,v=>{prefs.comicFit=v;savePrefs();}))
    )),
...
async function openComicReader(book){
  ...
  function rebuild(){
    ...
    if(mode==='webtoon'){
      ...
      const strip = h('div',{class:'comic-webtoon'+(fit==='height'?' fit-height':'')});
      ...
      bodyWrap.className='reader-body';
      const stage = h('div',{class:'comic-paged'+(fit==='width'?' fit-width':'')});
      const img = h('img',{});
      stage.append(img);
      const zones = h('div',{class:'tap-zones'},
        h('div',{class:'z',onclick:()=>goTo(cur-1)}),
        h('div',{class:'z mid',onclick:toggleHud}),
        h('div',{class:'z',onclick:()=>goTo(cur+1)}));
      bodyWrap.append(stage, zones);
      bodyWrap._img = img;
      showPage();
    }
  }
...
function comicSettingsPanel(mode, fit, onChange){
  ...
  const fitSeg = segPick(['width','height'],['너비','높이'],f,v=>{f=v;onChange(m,f);});
...
async function openEpubReader(book){
  ...
  }catch(e){ clear(area).append(h('div',{class:'center-pad'},'EPUB 뷰어 로딩 실패. 인터넷 연결을 확인하거나 자체 호스팅 설정을 확인하세요.')); return; }
  clear(area);
  bookObj = ePub(`/api/books/${b.id}/file`);
...
function textSettingsPanel(apply, onFlowChange, isTxt){
